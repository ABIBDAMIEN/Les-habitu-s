...
const ShopItem = ({ picture_url, chain, localisations, offers }: ShopApiItemType, onItemClick: () => void) => {
  ...
  function getReduction(percentage: string, amount: string) {
    console.log(parseInt(percentage), percentage)
  ...
  return (
    <div className="grid grid-cols-12 items-center bg-white rounded-lg py-5 my-3">
      ...
      <div className="col-span-2"> <p className="ml-3">{chain}</p></div>
      <div className="col-span-3">{localisations[0].address}</div>
      ...
      <div className="col-span-1">
        <button onClick={handleclick} className='rounded bg-green-600 my-auto ml-auto p-1'>
