...
const AddShopForm = ({ ItemToUpdate }) => {
	

	const defaultValues = {
		chain: ItemToUpdate.chain,
		adress: ItemToUpdate.localisations[0]?.address,
		picture_url: ItemToUpdate.picture_url,
		offers: [...ItemToUpdate.offers],
	};

	const FileName = ItemToUpdate.chain ? ItemToUpdate.chain.replace(/\s+/g, "_") + ".png" : "";
	...
	return (
		<form className="w-3/4 mx-auto py-3" onSubmit={handleSubmit(onSubmit)}>
			<>
			...
			<button className="mt-10 mb-5 ml-auto text-md font-semibold text-[#1A78A8] flex items-center" onClick={(e) => {
            e.preventDefault()
            append({ id: ItemToUpdate.offers.length + 1, amount: '', reduction: '' });
          }}>
				Ajouter une offre
			...
			<button type="submit" className="mx-4 w-full rounded text-white text-base font-semibold mt-10 bg-[#1A78A8] h-12 text-center">Enregistrer</button>
		</form>
	);
};
